...
| `vetexamt`, `enddate`, `statute` | Veteran's/other exemption amount, end date, statutory basis |
...
**Corrections to this table's own earlier inference, now grounded in official field definitions:** [NEMRC_GRANDLIST_EXPORT_AS_BUILT.md](NEMRC_GRANDLIST_EXPORT_AS_BUILT.md) §3–§4 examines a real NEMRC-to-Tax-Department Grand List export (Killington, tax year 2026) whose own `411_gl_HEADERS.csv` file — the Tax Department's official field dictionary for this data — is "close to the fields in final annual grand list as joined with parcel data by VCGI," per the Tax Department's own description. Three things in the table above should be corrected or refined against it:

1. **There is no single `expdesc` field.** The row above previously grouped `vetexamt`, `expdesc`, `enddate`, `statute` together; the official schema instead has **two separate numeric exemption-code fields** (`EXPCODE_SPEC` 1–9/16, `EXPCODE_STND` 1–7) plus `EXPSTATUTE` and `EXP_END` — no native text-description field exists upstream at all. If VCGI's product ever surfaces exemption text, it's derived by code lookup, not carried as its own column.
2. **`locaprop`'s data-quality problem is now sourced, not inferred, and has a concrete disposition:** per the Tax Department directly, it's "not well maintained in NEMRC, superseded by E911 Address" — and is **already removed from the current VTAX/VCGI export**, meaning the Grand List data VCGI actually receives is narrower than NEMRC's raw file, not just relabeled.
3. **`rescode` carries the same caveat directly from the Tax Department:** "DATA NOT WELL MAINTAINED" — worth treating any downstream use of it with the same caution already applied to `locaprop`.

See [NEMRC_GRANDLIST_EXPORT_AS_BUILT.md](NEMRC_GRANDLIST_EXPORT_AS_BUILT.md) for the full 45-field official schema, the two parallel exemption-code systems, the Municipal-GL/Education-GL exemption transformation matrix, and — most importantly for §5 below — the confirmed finding that genuine Active/Inactive parcel status is carried by only one file in this whole export family, and only for towns with a TIF district.
...
7. **The Tax Department's own NEMRC Grand List export structurally lacks Active/Inactive parcel status for the vast majority of towns.** Confirmed directly via a real sample ([NEMRC_GRANDLIST_EXPORT_AS_BUILT.md](NEMRC_GRANDLIST_EXPORT_AS_BUILT.md) §7): of the six-file NEMRC-to-Tax-Department export family, only one file (`411TFP`, TIF-by-parcel) carries a genuine Active/Inactive/Non-taxable status field (`PACTIVE`) — and that file exists **only for towns with a TIF district** (roughly 20-odd of Vermont's ~260 towns). For the remaining towns, none of `411_gl`, `411EXP`, or `411TOT` carries per-parcel active/inactive status at all. This means **VCGI's own separately, voluntarily collected parcel geometry (the published Active/Inactive GIS layers, §4) is currently the more complete statewide source of inactive-parcel awareness** — more complete than what the Tax Department itself receives via this channel for non-TIF towns. Since the Grand List module's own UI does track Active/Inactive status on every parcel regardless of TIF status (§1.5), the gap appears to be in what's *exported* to the Tax Department, not necessarily in what NEMRC's software tracks internally — but this materially affects how much the proposed `ADMINSPAN`/`GROUNDSPAN` model (§6) can lean on the Tax Department's existing Grand List data pipeline versus needing new export fields from NEMRC specifically for this purpose.
...
16. **Could non-TIF towns' NEMRC Grand List exports be extended to carry a `PACTIVE`-equivalent field?** Confirmed (§5 item 7, [NEMRC_GRANDLIST_EXPORT_AS_BUILT.md](NEMRC_GRANDLIST_EXPORT_AS_BUILT.md) §7) that genuine Active/Inactive/Non-taxable status is exported only for TIF-district towns today, even though the Grand List module's own UI tracks this status on every parcel regardless of TIF status (§1.5). Closing this export gap would materially reduce how much the `ADMINSPAN`/`GROUNDSPAN` model has to rely on VCGI's own voluntarily-submitted geometry as the source of truth for inactive-parcel status.
