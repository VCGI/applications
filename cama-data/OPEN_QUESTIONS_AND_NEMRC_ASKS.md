...
*This document consolidates every open question raised across this documentation set — previously scattered across seven documents' own "open questions" sections — into one cross-referenced index, organized by theme and by who would actually need to answer or act on each one. It also pulls out a short, standalone list of concrete changes NEMRC specifically would need to make to accommodate [VERMONT_CAMA_DATA_STANDARD_DRAFT.md](VERMONT_CAMA_DATA_STANDARD_DRAFT.md), given NEMRC's unusual dual role as both the statewide SPAN/Grand-List steward and one of four CAMA vendors (MicroSolve).*

*Nothing here is a new finding — every item is drawn directly from [SPAN_PARCEL_GRANDLIST_MODEL.md](SPAN_PARCEL_GRANDLIST_MODEL.md) §7 (22 items), [MSOL_AS_BUILT.md](MSOL_AS_BUILT.md) §8 (7 items), [PROVAL_AS_BUILT.md](PROVAL_AS_BUILT.md) §6 (4 items), [ASSESSPRO_AS_BUILT.md](ASSESSPRO_AS_BUILT.md) §6 (4 items), [NEMRC_GRANDLIST_EXPORT_AS_BUILT.md](NEMRC_GRANDLIST_EXPORT_AS_BUILT.md) §11 (9 items), [VTPIE/VTPIE_CAMA_REQUIREMENTS_PARCEL_ANALYSIS.md](VTPIE/VTPIE_CAMA_REQUIREMENTS_PARCEL_ANALYSIS.md) §8 (5 items), and [VERMONT_CAMA_DATA_STANDARD_DRAFT.md](VERMONT_CAMA_DATA_STANDARD_DRAFT.md) §9 (6 items) — 57 items total. Several were near-duplicates across documents (e.g., the `LOCAPROP` removal contradiction and the `HSTED_TX` drop each appear in two places); those are merged into one entry here with both sources cited, rather than listed twice. Each item below keeps a stable ID (`OQ-1`, `OQ-2`, ...) so the by-party index at the end can reference them without repeating full text. The original per-document sections are left intact — this is an additional index, not a replacement.*
...
| ID | Question | Responsible | Source |
|---|---|---|---|
| OQ-13 | What exactly flows back from CAMA to NEMRC's Grand List database (drawn bidirectional in VCGI's own diagram)? Partially answered for splits/transfers (confirmed manual, no auto-sync) — still open whether *any* transaction type syncs automatically. | NEMRC | [SPAN_PARCEL_GRANDLIST_MODEL.md §7](SPAN_PARCEL_GRANDLIST_MODEL.md#7-open-questions-for-the-ongoing-workgroup) item 8 |
...
| OQ-38 | Is `P_TZONE` genuinely meaningful internally to NEMRC's `lsprop`/`lssyst` tables, even though the exported `411TFP.TZONE` field is documented as unused? | NEMRC | [VTPIE_CAMA_REQUIREMENTS_PARCEL_ANALYSIS.md §8](VTPIE/VTPIE_CAMA_REQUIREMENTS_PARCEL_ANALYSIS.md#8-open-questions) item 4 |
